...
class AuthView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            showAssignReferalModal: false,
            statusDropdownDisable: false,
            authNumber: "",
            serviceType: "",
            patient_lob: "",
            assignedMember:"",
            ...
            showStatusChangeConfirmationModal: false,
            ...
            assigneeMember: "",
            role:"",
            showAccessWarningModal: false,
            showAuthStatusWarningModal:false,
            departments: [{
    ...
    componentDidMount(){
        ...
        .then(res => {
            ...
            this.setState({
                originalselectedstatus: res.data.json.status,
                selectedstatus: res.data.json.status,
                assignedMember: res.data.json.assigned_to,
                serviceType: res.data.json.service_type.service_type,
                patient_lob : res.data.json.p_lob,
                created_by: res.data.json.created_by,
                created_by_role: res.data.json.created_by_role,
                ...
                oralnotification: res.data.json.oral_notification
            });
    ...
    handleSelectStatusChange(text) {
        this.setState({
            selectedstatus: text,
            showAuthStatusWarningModal: !this.state.showAuthStatusWarningModal
        })
    }
    ...
    confirmReassignModal = () => {
        ...
        .then((response) => {
            this.setState({
                isLoaded: true,
                assignedMember: this.state.assigneeMemberEmail
            })
        })
    }
    ...
    updateReferal = () => {
        ...
        let patientid = localStorage.getItem('patientId');
            let createReferalData = {
                ...
                "assigned_to": this.state.assigneeMemberEmail, // assigned email ID
                "assigned_date": new Date(), 
                "created_date": this.state.created_date,
                "created_by": this.state.created_by,
                "created_by_role": this.state.created_by_role,
                "status": this.state.selectedstatus,
                "oral_notification": this.state.oralnotification === "Yes" ? true : false
            };
            axios({
                method: 'PUT',
    ...
    validateDetails = () => {
        // if(this.state.role.toLocaleLowerCase() === "supervisor"){
        //     this.updateReferal();
        // }else{
        //     if(this.state.originalselectedstatus === "OPEN" && this.state.assignedMember === this.state.caremanagerId){
        //         this.updateReferal();
        //     }else{
        //         this.setState({
        //             showAccessWarningModal: true
        //         })
        //     }
        // }
        if(this.state.selectedstatus === "OPEN" || this.state.selectedstatus === "APPROVED" || this.state.selectedstatus === "REJECTED" || this.state.selectedstatus === "CANCEL"){
            if(this.state.assignedMember === "UNASSIGNED"){
                this.setState({
                    showAssignReferalModal: true,
                    warningText: "Referal is not assigned. Please assign to yourself to make the changes"
                })
            }else if(this.state.assignedMember !== this.state.caremanagerId){
                this.setState({
                    showAssignReferalModal: true,
                    warningText: "Referal is assigned to " + this.state.assignedMember + ". Please re-assign yourself to make the changes"
                })
            }else if(this.state.assignedMember === this.state.caremanagerId && (this.state.selectedstatus === "OPEN" || this.state.selectedstatus === "APPROVED")){
                this.updateReferal();
            }else if(this.state.assignedMember === this.state.caremanagerId && this.state.role.toLocaleLowerCase() === "supervisor"){
                this.updateReferal();
            }
            else{
                this.setState({
    ...
    showAccessWarningModalToggle = () => {
        this.setState({
            showAccessWarningModal: !this.state.showAccessWarningModal
        })
    }
    ...
    //         this.setState({
    //             showAssignReferalModal: true,
    //             warningText: "Referal is not assigned to any user. Please re-assign yourself to make the changes"
    //         })
    //     }else if(this.state.assignedMember !== this.state.caremanagerId){
    //         this.setState({
    //             showAssignReferalModal: true,
    ...
    //     }
    //     // this.setState({
    //     //     showStatusChangeConfirmationModal: true
    //     // })
    // }
    showStatusChangeConfirmationModalToggle = () => {
        this.setState({
            showStatusChangeConfirmationModal: !this.state.showStatusChangeConfirmationModal
        })
    }
    showAssignReferalModalToggle = () => {
        this.setState({
            showAssignReferalModal: false
        })
    }
    confirmAssignReferalModalModalToggle = () => {
        this.setState({
            showAssignReferalModal: false,
            // statusDropdownDisable: true
        })
    }
    render() {
        console.log(this.state.assignedMember,this.state.role,"this.state.assignedMember")
        return (
            <React.Fragment>
                <Head>
                ...
                <Layout>
                    <MDBRow className="align-items-center" style={{marginBottom:"-10px"}}>
                        ...
                        <MDBCol sm="12" md="2" lg="2">
                            <div style={{ marginLeft: "0px" }}>
                                <MDBSelect
                                    options={this.state.oralnotifications}
                                    // selected = "Oral Notification"
                                    selected = {this.state.oralnotification === true && (this.state.selectedstatus === "APPROVED" || this.state.selectedstatus === "REJECTED" || this.state.selectedstatus === "CANCEL") ? "YES" : "Oral Notification"}
                                    disabled = {(this.state.serviceType === "Expedited/Urgent" && this.state.patient_lob === "Medicare" && this.state.selectedstatus !== "OPEN") ? false : true}
                                    // label="Oral Notification"
                                    // selected={this.state.oralnotification ? this.state.oralnotification : "Oral Notification"}
                                    getTextContent = {(text) => this.handleSelectOralNotificationChange(text)}
                                />
                            </div>
                        </MDBCol>

                        <MDBCol sm="12" md="2" lg="2">
                            <div style={{ marginLeft: "0px" }} onClick={this.handleDropdown}>
                                <MDBSelect
                                    options={this.state.status}
                                    // label="Status"
                                    // selected = "Status"
                                    // disabled = {this.state.assignedMember !== this.state.caremanagerId || !this.state.role === "Supervisor" ? true : false}
                            ...
                            <MDBRow style={{ marginTop: "12px", marginLeft: "12px", overflowY: "auto", maxHeight: "350px" }}>
                                {this.state.historyDetails && this.state.historyDetails.map((item) => {
                                    ...
                                    <div className="historycontent">
                                        ...
                                        {item.auth_status === "OPEN" ? <p style={{ color: "#424242", fontSize: "14px", lineHeight: "19px" }}><span style={{ color: "#4CAF50", fontWeight: "600" }}>{item.modified_by + " , " + item.modified_user_role}</span> assigned to <span style={{ color: "#536DFE", fontWeight: "600" }}>{item.assigned_to + " , "+ item.assigned_to_user_role}</span></p> :""}
                                        {item.auth_status === "APPROVED" ? <p style={{ color: "#424242", fontSize: "14px", lineHeight: "19px" }}><span style={{ color: "#4CAF50", fontWeight: "600" }}>{item.modified_by + " , " + item.modified_user_role}</span> Approved the referral </p> :""}
                                        {item.auth_status === "REJECTED" ? <p style={{ color: "#424242", fontSize: "14px", lineHeight: "19px" }}><span style={{ color: "#4CAF50", fontWeight: "600" }}>{item.modified_by + " , " + item.modified_user_role}</span> Rejected the referral</p> :""}
                                        {item.auth_status === "CANCEL" ? <p style={{ color: "#424242", fontSize: "14px", lineHeight: "19px" }}><span style={{ color: "#4CAF50", fontWeight: "600" }}>{item.modified_by + " , " + item.modified_user_role}</span> Canceled the referral</p> :""}
                    ...
                    <MDBModal isOpen={this.state.showAccessWarningModal} toggle={this.showAccessWarningModalToggle} className="create-referal-modal">
                        <MDBModalHeader className="modaltitle">Notice</MDBModalHeader>
                        <MDBModalBody>
                            <div className="modalconfirmationtitle text-left" style={{ marginLeft: "4px", marginTop: "-20px" }}>Only Supervisors have access to modify the referral</div>
                            <div className="text-right">
                                {/* <MDBBtn flat className="flatbutton" onClick={this.showConfirmDeletetoggle}>Cancel</MDBBtn> */}
                                <MDBBtn flat className="flatbutton" onClick={this.showAccessWarningModalToggle}>ok</MDBBtn>
                            </div>
                        </MDBModalBody>
                    </MDBModal>
                    {/* <MDBModal isOpen={this.state.showStatusChangeConfirmationModal} toggle={this.showStatusChangeConfirmationModalToggle} className="create-referal-modal">
                        <MDBModalHeader className="modaltitle">Confirmation</MDBModalHeader>
                        <MDBModalBody>
                            <div className="modalconfirmationtitle text-left" style={{ marginLeft: "10px", marginTop: "-20px" }}>Are you sure you want to choose it?</div>
                            <div className="text-right">
                                <MDBBtn flat className="flatbutton" onClick={this.showStatusChangeConfirmationModalToggle}>Cancel</MDBBtn>
                                <MDBBtn flat className="flatbutton" onClick={this.confirmStatusChangeConfirmationModalToggle}>Yes</MDBBtn>
                            </div>
                        </MDBModalBody>
                    </MDBModal> */}
                    <MDBModal isOpen={this.state.showAssignReferalModal} toggle={this.showAssignReferalModalToggle} className="create-referal-modal">
                        <MDBModalHeader className="modaltitle">Notice</MDBModalHeader>
                        <MDBModalBody>
                            <div className="modalconfirmationtitle text-left" style={{ marginLeft: "10px", marginTop: "-20px" }}>{this.state.warningText}</div>
                            <div className="text-right">
                                {/* <MDBBtn flat className="flatbutton" onClick={this.showAssignReferalModalToggle}>Cancel</MDBBtn> */}
                                <MDBBtn flat className="flatbutton" onClick={this.confirmAssignReferalModalModalToggle}>ok</MDBBtn>
                            </div>
                        </MDBModalBody>
                    </MDBModal>

                    <MDBModal isOpen={this.state.showAuthStatusWarningModal} toggle={this.authStatusUpdateModalToggle} className="newauthviewmodal">
                        <MDBModalHeader className="modaltitle" toggle={this.authStatusUpdateModalToggle}>Confirmation</MDBModalHeader>
                        <MDBModalBody className="modalbody" style={{padding: "0rem 1rem"}}>
                            <p className="confirmationmodalmessage">Are you sure you want to choose it?</p> 
                        </MDBModalBody>
                        <MDBModalFooter>
                            <MDBBtn flat className="flatbutton" onClick={this.authStatusUpdateModalToggle}>Cancel</MDBBtn>
                            <MDBBtn flat className="flatbutton" onClick={this.authStatusUpdateModalToggle}>YES</MDBBtn>
                        </MDBModalFooter>
                    </MDBModal>
